...
M<-cbind(live.with.all.child,weekly.contact.child,trans.from.child,pension,med.insur,commu.dance,commu.card.chess,commu.out.exer,commu.care.center,commu.assoc,commu.act.center,commu.org.help)
colnames(M)<-colnames(cbind(data[,24:26],data[,29:37]))
...
ind<-sample(nrow(newdata),8161,replace = F)
...
M<-matrix(M,8161,12,byrow = F)

C<-as.numeric(unlist(C))
C<-matrix(C,8161,20,byrow = F)
...
#scalem=apply(M,2,sd)
centerm=matrix(0,1,12)
scalem=matrix(1,1,12)
MM=M
#for (i in 1:pM) {
...
beta0_m[is.na(beta0_m)]=0
beta0_y[is.na(beta0_y)]=0
...
save(returnlist,file = "sample3000.RData")
...
#colname
A1<-as.matrix(returnlist[["beta_m"]][1,1,1,,])
colnames(A1)<-colnames(cbind(X,C))

A2<-as.matrix(returnlist[["beta_m"]][1,1,2,,])
colnames(A2)<-colnames(cbind(X,C))

B1<-t(as.matrix(returnlist[["beta_y"]][1,1,1,]))
colnames(B1)<-colnames(cbind(X,M,C))

B2<-t(as.matrix(returnlist[["beta_y"]][1,1,2,]))
colnames(B2)<-colnames(cbind(X,M,C))

intersect(which(cluster[1,1,]==1),g1)
intersect(which(cluster[1,1,]==2),g2)


length(intersect(which(cluster[1,1,]==1),g1))/length(which(cluster[1,1,]==1))
length(intersect(which(cluster[1,1,]==2),g2))/length(which(cluster[1,1,]==2))
#length(intersect(which(cluster[1,1,]==3),g3))/length(which(cluster[1,1,]==3))
#length(intersect(which(cluster[1,1,]==4),g4))/120
#length(intersect(which(cluster[1,1,]==5),g5))/120