...
#y<-matrix(y,8161,1,byrow = F)

test.data <- list(X,M,C,y)
...
W=kronecker(weight_m, diag(1,(pT+pC)))     #### Enlarged weighting matrix with Dimension: (pM*(1+pT+pC)) x (pM*(1+pT+pC))
...
cluster1<-which(cluster0==1)
cluster2<-which(cluster0==2)
cluster3<-which(cluster0==3)

sample1<-sample_data[cluster1,]
sample2<-sample_data[cluster2,]
sample3<-sample_data[cluster3,]

A1<-t(as.matrix(round(beta0_y[1,],4)))
colnames(A1)<-colnames(cbind(X,M,C))
A2<-t(as.matrix(round(beta0_y[2,],4)))
colnames(A2)<-colnames(cbind(X,M,C))
A3<-t(as.matrix(round(beta0_y[3,],4)))
colnames(A3)<-colnames(cbind(X,M,C))


cluster_all<-list(cluster1,cluster2,cluster3,XX,MM,CC,yy)

#bh<-array(0, c(k, pM, pT))
#Gamma<-array(0, c(1, pM, pC))
#betah<-matrix(0, k, pT)
#thetah<-matrix(0, k, pM)
#gamma<-matrix(0, 1, pC)


LOSS<-function(initial_value,cluster_all){
  ...
  bh<-array(initial_value[1:12], c(k, pM, pT))
  Gamma<-array(initial_value[13:92], c(1, pM, pC))
  betah<-matrix(initial_value[93:95], k, pT)
  thetah<-matrix(initial_value[96:107], k, pM)
  gamma<-matrix(initial_value[108:127], 1, pC)  
  ...
  cluster1<-cluster_all[[1]]
  cluster2<-cluster_all[[2]]
  cluster3<-cluster_all[[3]]
  
  #cluster1
  res0_m1=MM[cluster1,]-data_m[cluster1,]%*%t(cbind(bh[1,,],Gamma[1,,]))
  res0_y1=yy[cluster1]-data_y[cluster1,]%*%c(betah[1,],thetah[1,],gamma[1,])
  ploss1<-sum(diag(res0_m1%*%weight_m%*%t(res0_m1))+weight_y*(res0_y1)^2)
  
  #cluster2
  res0_m2=MM[cluster2,]-data_m[cluster2,]%*%t(cbind(bh[2,,],Gamma[1,,]))
  res0_y2=yy[cluster2]-data_y[cluster2,]%*%c(betah[2,],thetah[2,],gamma[1,])
  ploss2<-sum(diag(res0_m2%*%weight_m%*%t(res0_m2))+weight_y*(res0_y2)^2)
  
  #cluster3  
  res0_m3=MM[cluster3,]-data_m[cluster3,]%*%t(cbind(bh[3,,],Gamma[1,,]))
  res0_y3=yy[cluster3]-data_y[cluster3,]%*%c(betah[3,],thetah[3,],gamma[1,])
  ploss3<-sum(diag(res0_m3%*%weight_m%*%t(res0_m3))+weight_y*(res0_y3)^2)
  
  ploss_all<-ploss1+ploss2+ploss3
  return(ploss_all)
}

initial_value<-matrix(0,1,127)
#optim(initial_value,LOSS,cluster_all=cluster_all)

para<-optim(initial_value,LOSS,cluster_all=cluster_all)$par

beta0_m<-array(0,c(k,pM,pT+pC))
beta0_y<-matrix(0,k,pT+pM+pC)

bh<-array(para[1:12], c(k, pM, pT))
Gamma<-array(para[13:92], c(1, pM, pC))
betah<-matrix(para[93:95], k, pT)
thetah<-matrix(para[96:107], k, pM)
gamma<-matrix(para[108:127], 1, pC)

beta0_m[1:3,1:4,1]<-bh
beta0_m[1,1:4,2:21]<-Gamma
beta0_m[2,1:4,2:21]<-Gamma
beta0_m[3,1:4,2:21]<-Gamma
  
beta0_y[1:3,1]<-betah
beta0_y[1:3,2:5]<-thetah
beta0_y[1,6:25]<-gamma
beta0_y[2,6:25]<-gamma
beta0_y[3,6:25]<-gamma
...
lambda1<-c(0.0166923645,0.0178986753)
...
save(returnlist,file = "sample_6.RData")
